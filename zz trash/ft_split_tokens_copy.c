...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_split_tokens copy.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: rdel-olm <rdel-olm@student.42malaga.com>   +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/31 11:01:14 by rdel-olm          #+#    #+#             */
/*   Updated: 2024/11/15 21:31:14 by rdel-olm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_token	*ft_split_tokens(t_token *token)
{
    ...
    if (strchr(token_value, '|'))
    {
        ...
        while (sub_tokens[i])
        {
            ...
            // Crear un nuevo token si hay más sub_tokens
            if (sub_tokens[i])
            {
                t_token *new_token = ft_create_new_token(current, sub_tokens[i]);
                if (!new_token)
                {
                    ft_free_split(sub_tokens);
                    return (NULL);
    ...
    // Iterar sobre la lista enlazada sin modificar tokens con '-'
    current = token;
    while (current)
    {
        // Si el token contiene un "-", no hacer nada y pasar al siguiente
        if (strchr(current->token_value, '-'))
        {
            current = current->next;
            continue;
        }

        // Si necesitas otra lógica de separación para tokens específicos, puedes agregarla aquí.
